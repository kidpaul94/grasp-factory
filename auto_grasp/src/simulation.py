...
class Auto_grasp():
    ...
    def initialize(self) -> None:
        ...
        for obj in self.sdf_names:
            ...
            centers, directions, aprvs = grasp_gen(path=f'{patch_dict}/cpps.txt')

            for idx in tqdm(range(len(directions))):
                res = self.execute(centers[:,idx], directions[:,idx], obj, aprvs[idx])
                if res >= 0:
